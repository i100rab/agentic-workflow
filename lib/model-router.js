...
// Length/structure heuristic - not a trained classifier, a starting point.
// Pass your own via createOptimizedClient's opts.classify if you have
// domain knowledge about what makes a call complex in your pipeline.
//
// Thresholds are calibrated for real production prompts, not toy examples:
// a one-word classification is a few dozen characters, but a genuine
// "summarize this brief" or "review this draft" call is routinely several
// thousand characters and still perfectly fine for a mid-tier model. Only
// prompts that are both very long AND carry real structural complexity
// (code, schemas, explicit multi-step reasoning) get pushed to capable -
// sheer bulk alone (e.g. a wall of pasted research text) doesn't.
//
// One real constraint this doesn't check: smaller/cheaper models often have
// smaller context windows than Claude. A call classified "mid" or "cheap"
// still needs to actually fit in that provider's context limit - verify
// your routing table's models can handle your largest realistic prompt
...
export function classifyComplexity(text) {
  ...
  if (s.length < 500 && !hasMultiStep && !hasStructured) return "cheap";
  if (s.length < 8000 && !hasStructured) return "mid";
  return "capable";
}
