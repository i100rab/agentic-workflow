...
import { routeCall, callProvider, costOfRoutedCall, classifyComplexity as defaultClassify, DEFAULT_ROUTING_TABLE } from "./model-router.js";
...
/**
 * @param {string|Object} config - either an Anthropic API key (string, backward
 *   compatible with every existing call site), or a keys object
 *   { anthropic, openai, groq } to enable multi-provider routing.
 * @param {Object} [opts]
 * @param {Object} [opts.routingTable] - tier -> {provider, model}, defaults to
 *   cheap=Groq Llama, mid=GPT-4o mini, capable=Claude Sonnet
 * @param {Function} [opts.classify] - (promptText) => "cheap"|"mid"|"capable"
 * @param {Object} [opts.pricing] - override default $/1K token estimates
 */
export function createOptimizedClient(config, opts = {}) {
  const keys = typeof config === "string" ? { anthropic: config } : config;
  const real = new Anthropic({ apiKey: keys.anthropic });
  ...
  const routingTable = opts.routingTable ?? DEFAULT_ROUTING_TABLE;
  ...
  async function create(params) {
    ...
    // Routing decision happens before the cache lookup so the bucket is keyed
    // per resolved model, not just per agent role - a Groq response and a
    // Sonnet response to the same question aren't interchangeable, so they
    ...
    const bucket = `${bucketKeyFor(params.system)}::${decision.model}`;
    ...
    if (best && bestScore >= similarityThreshold) {
      ...
      return {
        ...
        __optimized: { cacheHit: true, matchScore: Number(bestScore.toFixed(2)), routing: decision },
      };
    }
    ...
    const { routing: _routing, ...anthropicParams } = params;

    const response =
      decision.provider === "anthropic"
        ? await real.messages.create({ ...anthropicParams, model: decision.model, messages: compressed })
        : await callProvider(decision.provider, keys, {
    ...
    if (params.routing) {
      stats.routedCalls++;
      stats.routingByTier[decision.tier] = (stats.routingByTier[decision.tier] || 0) + 1;
      // What this call would have cost on the "capable" tier (Sonnet), so the
      // dashboard can show routing's own savings separate from cache/compression.
      const baselineModel = routingTable.capable.model;
      const baselineCost = costOfRoutedCall(baselineModel, response.usage, pricing);
      const actualCost = costOfRoutedCall(decision.model, response.usage, pricing);
      stats.costSavedByRouting += Math.max(0, baselineCost - actualCost);
    }
    ...
    return {
      ...response,
      __optimized: { cacheHit: false, tokensSavedByCompression: tokensSaved, routing: decision },
    };
  }
